...
const { getdayw } = require("./daywiseController");
...
const papercodewiseController = {
  ...
  getexamdayw: async (req, res) => {
    ...
    institutesConnection.query(
      `SELECT distinct(exam_dayw) as exam_dayw FROM s24_timetable_subjects_course_all where paper_code=${papercode} and block='N' ORDER BY paper_code`,
      (error, results, fields) => {
        if (error) {
          console.error("Error querying MySQL:", error);
          res.status(500).send("Error fetching data from MySQL");
